...
sub sig_action {
  ...
  if(!$color) {
    foreach my $n (keys %saved_colors) {
      if(lc($n) eq lc(substr($nick, 0, length($n)))) { $color = $saved_colors{$n}; }
    }
  }

  if($msg =~ /sarg/i) { $color = 5; }
  if($msg =~ /besen/i) { $color = 5; }

  if (!$color) {
    $server->command('/^format action_public %n{pubaction $0}$1');
  } else {
    $color = "0".$color if ($color < 10);
    $server->command('/^format action_public '.chr(3).$color.'{pubaction $0}$1');
  }
...
sub sig_public {
  ...
  if (!$color) {
    $server->command('/^format pubmsg %n{pubmsgnick $2 {pubnick $0}}$1');
  } else {
    $color = "0".$color if ($color < 10);
    $server->command('/^format pubmsg '.chr(3).$color.'{pubmsgnick $2 {pubnick $0}}$1');
  }
}
